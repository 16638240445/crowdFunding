...
function getProjectType() {
	$.ajax({
		url : pageContext + "/publish/find.action",
		method : "post",
		dataType : "json", // xml text json
		async : true,
		success : function(data) {
			for (var i = 0; i < data.length; i++) { // <option>公益</option>
				var option = "<option value=\"" + data[i].pstId + "\"";
				option += ">" + data[i].pstName + "</option>"; // 动态添加数据
				
				$("select[name=projectTypeId]").append(option);
